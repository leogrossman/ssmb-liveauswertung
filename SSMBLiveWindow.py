...
class SSMBWindow(tk.Frame):
    ...
    def __init__(self, master=None, testing=False, plotting=True):
        ...
        # self.__rawdata_numsavedEtr.grid(row=6, column=5, padx=3)
    ...
    def startup(self):
        ...
        ### read turn for autoscale parameters:
        for hi in [1,2]:
            ...
            try:
                ...
                print('Information: SSMBLiveAnalyzer parameter "%s" not in configuration file "%s"' % (parametername, self.__cfg_name))
            except ValueError:
                print('Warning: SSMBLiveAnalyzer parameter "%s" with invalid value in configuration file "%s"' % (parametername, self.__cfg_name))

        ### read chunk restart watch PVs:
        parametername = 'chunk_watch_pvs'
        parameterlabel = 'chunk_watch_PVs'
        try:
            parameters.update({parameterlabel: self._cfg[self.__cfg_key][parametername].split(',')})
        except KeyError:
            print('Information: SSMBLiveAnalyzer parameter "%s" not in configuration file "%s"' % (parametername, self.__cfg_name))
        except ValueError:
            print('Warning: SSMBLiveAnalyzer parameter "%s" with invalid value in configuration file "%s"' % (parametername, self.__cfg_name))
        ...
        try:
            ...
            self._ctrl = SSMBScopeControl(scopeip, self._analyzer.data_queue, self._analyzer.maxvalue_queue,
                                          active_channels=[self.__column_harm1, self.__column_harm2, self.__column_laser, self.__column_trigger],
                                          active_maths=[self.__column_math1, self.__column_math2], timename=self.__column_time,
                                          autoset_time=True)
    ...
    def refresh_acquisition(self, acqstate, acqnumber, savestatus, displaystatus, triggermode, triggerflank, mathavglen, seqlen = None):
        ...
        if self.__harm1shown:
            self.__harm1showBtn.configure(text='Trace shown', bg=self.__color_btndefault, activebackground=self.__color_btndefault)
        else:
            self.__harm1showBtn.configure(text='Trace hidden', bg=self.__color_btnyellow, activebackground=self.__color_btnyellow)
        
        self.__harm2shown = displaystatus[self.__column_harm2]
        if self.__harm2shown:
